---
title: "      Modelos lineales de Efecto Mixtos
                 
                 Metodos Estadisticos Avanzados  
                 
                 Margarita Hernandez Montoya"
...
---
URL del GITHUB
https://github.com/mmhernandm/Metodos-avanzados-EC1209/tree/master
...
Al revisar su estructura interna se encuentra que está formado por un archivo txt para cada estado financiero.
Se extrae el archivo de estado de resultados integrales ERI, denominado EstadoResultadosIntegrales(ERI)XXXX.txt.
La estructura tiene los datos de identificación de las empresas en el mismo archivo txt que las columnas de lo estados financieros, se abre en excel y es texto delimitado sin tabuladores así que se descarga sin inconvenientes.  Para extraer la información se ejecutaron los siguientes pasos en Excel:
...
La búsqueda de los indicadores Macroeconomicos fue desgastante al principio porque en general las páginas tienen muchos enlaces a datos viejos o  a veces no es muy clara la moneda o exactamente a qué se refieren las diferentes variedades del indicador y no se conocía la página de los boletines del Banrep.  Adicionalmente, se tenían al inicio los indicadores de 2015 al 2018 y al ampliar la muestra hasta el 2012 fue necesario también complementar estos años.
  ...
  * Inflación
  ...
  de ahí se extractaron ya los datos oficiales. En el dataset se usa como proporción no como porcentaje.
...
NOTA : para evitar problemas por el uso de mezclas de mayúsculas y minúsculas, en adelante se usarán todas las siglas en
minúscula para referirse a las variables del código fuente.
...
Se tienen 20 NIT con registros completos de 7 años y uno con 6 años, puede decirse que está muy balanceada en cuanto a NIT
ya que
...
casa matriz y  la clasificación industrial internacional .A continuación se ilustra el balance de la muestra respecto a estos factores:
...
Se buscó completar una muestra con la mayor cantidad posible de registros por lo cual se excluyeron los NIT con menos
de 6 registros de estados financieros entre 2012-2018.  
Esta exclusión busca disminuir que entren nuevos NIT pero pocos registros y así evitar fallas al calcular el número de efectos de los modelos por tener muchos efectos y pocos datos.

Al fijar el número de NIT y de registros por NIT, la muestra también quedará balanceada en datos por año.
...
Este sector en adelante se denominará consultoría.
...
Respecto a la categoría ciiu vista desde año, la muestra está desbalanceada porque en los 146 registros dominan 
las empresas de consultoría y de administración de sistemas (J6202),  el resto que son los desarrolladores (J6201)
...
A continuación se baja al detalle de la gráfica 2, para entrar a revisar por periodo y NIT pero graficando el cociente de los costos/ingresos que puede de forma mas directa mostrar el desempeño ya que siempre se desa
...
Dee ambas gráficas se ve que en las colas inferiores a 0.5 y superiores a 1, hay unos pocos datos siendo mayor cantidad los de las colas inferiores a 0.5, entonces hay un poco mas de empresas con resultados my buenos que de empresas con resultados muy malos.
...
Hay 21 NIT en el dataset de empresas con registros completos y usando el modelo ajustado con la
...
El modelo ajusta bien para todos los NIT pero deja el tiempo por fuera.
...
estándar. Adicionalmente, tiene efectos aleatorios de los NIT y el periodo.  Los efectos aleatorios tienen una
...
El predicho de costos/ingresos mantiene la tendencia que se habia detectado antes así:
  ...
  * Suave pendiente positiva del J6209 servicios de software comercial: implica menor 
    productividad, podrán tener mas costos de ventas o menos ingresos.
  ...
  * Casi estabilidad para los del tipo J6311 Infraestructura TI, los reales muestran tendencia
    un poco más marcada. Se mantendrá su desempeño. 
...
conocidos. Es interesante ver cómo las empresas de consultoría en general van perdiendo sus márgenes y 
hace pensar en que hay que diversificar y asesorar a las empresas no tanto
en lo tradicional sino en el desarrollo de aplicaciones(J6201).El tipo de industria J6311 sorprende
por su bajo margen.  Estos hallazgos merecen mayor profundización para el que quiere emprender, invertir o
dar créditos.
...
de regresión converjan, si las variables tienen escalas muy disímiles puede ocurrir que durante la ejecución el método 
mismo saque error por escala.  Esta estandarización debe armonizar los comportamientos de las variables 
respetando los signos que les son propios, no sobretransformando porque se debe mantener la facilidad de interpretación y
...
*  Si bien estos modelos se supone que soporta, muestras pequeñas y desbalanceadas, igual que cualquier otro
método fallan cuando la escasez de datos es extrema, así al principio cuando se tenían sólo 3 ó 4 datos para entrenar,
todos los modelos fallaban sólo quedaban lo interceptos o en otros casos de generaban errores por por valores propios negativos e imposibilidad de convergencia.
...
habia mas del 80% de NIT con un sólo regitro, esto ocasionaba que no hubiera suficientes datos para calcular los efectos , la razón era que por cada individuo hay que multiplicar por 2 ( una pendiente + un intercepto) para hallar el número de efectos y este factor puede aumentar si hay mas varibles,entonces cuando la función determina si se tienen suficientes datos
para los efectos puede fallar facilmente por número insuficiente de datos.

* La librería nlme es mas rica en información para los modelos porque da el AIC, BIC, t, p, intervalos de 
confianza , pero su sintáxis para los modelos mixtos es menos clara para interpretar que la de lme4.
